...
class FixedCapBiMap {
	...
	//removes a key k with all the values that belong to it and returns ValuePair containing the removed values 
	//if k has 2 values, these values are in ValuePair, if it has only 1 value the first component is ValuePair, the second is NULL_VALUE
	//if k has no associated values, both components are NULL_VALUE
